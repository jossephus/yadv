...
export const unstagedDiffAtom = gitRuntime.atom(GitService.use((git) => git.unstagedDiff()))
export const stagedDiffAtom = gitRuntime.atom(GitService.use((git) => git.stagedDiff()))
export const readUnstagedDiffAtom = gitRuntime.fn<void>()(() => GitService.use((git) => git.unstagedDiff()))
export const readStagedDiffAtom = gitRuntime.fn<void>()(() => GitService.use((git) => git.stagedDiff()))
...
export const stagePatchAtom = gitRuntime.fn<string>()((patch) => GitService.use((git) => git.stagePatch(patch)))
export const unstagePatchAtom = gitRuntime.fn<string>()((patch) => GitService.use((git) => git.unstagePatch(patch)))
